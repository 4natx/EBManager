    ...
    for (let i = 0; i < files.length; i++) {
        ...
        if (!file.includes(".")) {
            ...
            try {
                ...
                console.error(
                    `Couldn't load the command data for the ${file.split(".")[0]} command with error: ${e}`,
                );
            }
        }
    }
...
export async function registerSlashCommands(reload?: boolean) {
    ...
    if (config.groupIds.length === 0)
        config.lockedCommands = config.lockedCommands.concat(
            CommandHelpers.getGroupCommands(),
        );
    if (config.universes.length === 0)
        config.lockedCommands = config.lockedCommands.concat(
            CommandHelpers.getGameCommands(),
        );
    if (!config.xpSystem.enabled)
        config.lockedCommands = config.lockedCommands.concat(
            CommandHelpers.getXPCommands(),
        );
    if (!config.counting.enabled) config.lockedCommands.push("setgoal");
    for (let i = 0; i < commands.length; i++) {
        let lockedCommandsIndex = config.lockedCommands.findIndex(
            (c) => c.toLowerCase() === commands[i].name,
        );
        let allowedCommandsIndex = CommandHelpers.allowedCommands.findIndex(
            (c) => c.toLowerCase() === commands[i].name,
        );
        if (lockedCommandsIndex !== -1 && allowedCommandsIndex === -1) {
            BetterConsole.log(
                `Skipped registering the ${commands[i].name} command because it's locked and not part of the default allowed commands list`,
            );
            continue;
        }
        ...
        try {
            ...
            console.error(
                `Couldn't load the slash command data for the ${commands[i].name} command with error: ${e}`,
            );
        }
    }
    ...
    try {
        ...
        console.error(
            `There was an error while registering slash commands: ${e}`,
        );
    }
}
...
async function deleteGuildCommands() {
    ...
    for (let i = 0; i < guilds.size; i++) {
        ...
        try {
            ...
            console.error(
                `There was an error while trying to delete guild commmands: ${e}`,
            );
        }
    }
}
...
export async function loginToRoblox(robloxCookie: string) {
    ...
    BetterConsole.log(
        `Logged into the Roblox account - ${client.robloxInfo.UserName}`,
        true,
    );
...
client.once("ready", async () => {
    BetterConsole.log(
        `Logged into the Discord account - ${client.user.tag}`,
        true,
    );
        ...
        if (commands[i].name === command) {
            try {
                await interaction.deferReply({
                    ephemeral: commands[i].commandData.isEphemeral,
                });
            } catch (e) {
                console.error(e);
            ...
            if (args["username"]) {
                ...
                if (usernames.length > config.maximumNumberOfUsers) {
                    let embed = client.embedMaker({
                        ...
                        description:
                            "Você inseriu mais usuários do que o máximo permitido no momento, abaixe a quantidade de usuários em seu comando e tente novamente",
                        type: "error",
            ...
            if (
                !CommandHelpers.checkPermissions(
                    commands[i].file,
            ...
            if (commands[i].file.commandData.hasCooldown) {
                if (
                    client.isUserOnCooldown(
                        commands[i].file.slashData.name,
                        interaction.user.id,
                    )
                ) {
                    let embed = client.embedMaker({
                        title: "Cooldown",
                        description:
                            "Você está atualmente em cooldown para este comando, tome uma chocolate quente enquanto isso",
                        type: "error",
                        author: interaction.user,
                    });
                    await interaction.editReply({ embeds: [embed] });
            ...
            if (commands[i].file.commandData.preformGeneralVerificationChecks) {
                ...
                let robloxID = await VerificationHelpers.getRobloxUser(
                ...
                if (robloxID !== 0) {
                    verificationStatus =
                        await VerificationHelpers.preformVerificationChecks(
                            groupID,
                            robloxID,
                            commands[i].commandData.permissionToCheck,
                        );
                } else {
                    verificationStatus = {
                        success: false,
                        err: `User is not verified with the configured verification provider (${config.verificationProvider})`,
                    };
                }
            ...
            try {
                ...
                let embed = client.embedMaker({
                    title: "Error",
                    description:
                        "Houve um erro ao tentar executar esse comando. O erro foi registrado no console",
                    type: "error",
                    author: interaction.user,
                });
                await interaction.editReply({ embeds: [embed] });
            ...
            if (commands[i] && commands[i].file.commandData.hasCooldown) {
                ...
                if (typeof res === "number") {
                    ...
                    client.commandCooldowns.push({
                        ...
                        userID: interaction.user.id,
                        cooldownExpires:
                            Date.now() + commandCooldown * usernames.length,
                    });
                } else {
                    client.commandCooldowns.push({
                        commandName: commands[i].file.slashData.name,
...
client.on("messageCreate", async (message: Discord.Message) => {
    ...
    let xpData = JSON.parse(
        await fs.promises.readFile(
            `${process.cwd()}/database/xpdata.json`,
            "utf-8",
        ),
    ) as UserEntry[];
...
client.on(
    "messageReactionAdd",
    async (reaction: Discord.MessageReaction, user: Discord.User) => {
        if (!config.xpSystem.enabled) return;
        if (user.bot) return;
        let xpData = JSON.parse(
            await fs.promises.readFile(
