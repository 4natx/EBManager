...
const command: CommandFile = {
    run: async(interaction: Discord.CommandInteraction, client: BotClient, args: any): Promise<any> => {
        ...
        await client.logAction(`<@${interaction.user.id}> começou a aceitar todas as solicitações no grupo pelo motivo: **${reason}** no grupo **${GroupHandler.getNameFromID(groupID)}**`);
        try {
    ...
    .addStringOption(o => o.setName("group").setDescription("O grupo para aceitar todas as solicitações").setRequired(true).addChoices(...GroupHandler.parseGroups() as any))
    .addStringOption(o => o.setName("reason").setDescription("A razão pela qual você está aceitando todos esses pedidos").setRequired(true)) as Discord.SlashCommandBuilder,
    commandData: {
