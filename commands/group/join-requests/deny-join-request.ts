...
const command: CommandFile = {
    run: async(interaction: Discord.CommandInteraction, client: BotClient, args: any): Promise<any> => {
        ...
        for(let i = 0; i < usernames.length; i++) {
            ...
            await client.logAction(`<@${interaction.user.id}> negou a solicitação de **${username}** pelo motivo de **${reason}** em **${GroupHandler.getNameFromID(groupID)}**`);
            continue;
        }
    ...
    .addStringOption(o => o.setName("group").setDescription("O grupo a negar a solicitação").setRequired(true).addChoices(...GroupHandler.parseGroups() as any))
    ...
    .addStringOption(o => o.setName("reason").setDescription("O (s) motivo (s) de por que você está negando as solicitações").setRequired(false)) as Discord.SlashCommandBuilder,
    commandData: {
