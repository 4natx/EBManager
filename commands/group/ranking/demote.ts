...
const command: CommandFile = {
    run: async(interaction: Discord.CommandInteraction<Discord.CacheType>, client: BotClient, args: any): Promise<any> => {
        ...
        for(let i = 0; i < usernames.length; i++) {
            ...
            await client.logAction(`<@${interaction.user.id}> rebaixou **${username}** de **${oldRoleName}** para **${potentialRole.name}** pelo motivo de **${reason}** em **${GroupHandler.getNameFromID(groupID)}**`);
        }
    ...
    .addStringOption(o => o.setName("group").setDescription("O grupo para fazer o rebaixamento em").setRequired(true).addChoices(...GroupHandler.parseGroups() as any))
    ...
    .addStringOption(o => o.setName("reason").setDescription("O (s) motivo (s) do (s) rebaixamento (s)").setRequired(false)) as Discord.SlashCommandBuilder,
    commandData: {
        category: "Ranking",
        isEphemeral: false,
