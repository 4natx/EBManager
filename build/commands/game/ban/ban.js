...
const command = {
    run: async (interaction, client, args) => {
        ...
        for (let i = 0; i < usernames.length; i++) {
            ...
            let res = await BanService_1.default.ban(universeID, robloxID, reason);
            if (res.err) {
                logs.push({
                    username: username,
                    status: "Error",
            ...
            if (config_1.default.ban.banDiscordAccounts) {
                let discordIDs = await VerificationHelpers_1.default.getDiscordUsers(interaction.guild.id, robloxID);
                BetterConsole_1.default.log(`Fetected Discord IDs for Roblox ID ${robloxID}: ${discordIDs}`);
    ...
    slashData: new discord_js_1.default.SlashCommandBuilder()
        ...
        .addStringOption(o => o.setName("universe").setDescription("O universo para executar esta ação em").setRequired(true).addChoices(...UniverseHandler_1.default.parseUniverses()))
        .addStringOption(o => o.setName("username").setDescription("The username(s) of the user(s) you wish to ban").setRequired(true))
        .addStringOption(o => o.setName("reason").setDescription("The reason(s) of the bans(s)").setRequired(false)),
    commandData: {
        category: "Ban",
        isEphemeral: false,
