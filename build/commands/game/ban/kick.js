...
const command = {
    run: async (interaction, client, args) => {
        ...
        for (let i = 0; i < usernames.length; i++) {
            ...
            try {
                await MessagingService_1.default.sendMessage(universeID, "Kick", { username: username, reason: reason });
            }
            catch (e) {
                logs.push({
                    username: username,
    ...
    slashData: new discord_js_1.default.SlashCommandBuilder()
        ...
        .addStringOption(o => o.setName("reason").setDescription("The reason(s) of the kick(s)").setRequired(false)),
    commandData: {
        category: "Ban",
        isEphemeral: false,
        permissions: config_1.default.permissions.game.kick,
        hasCooldown: true,
