...
const command = {
    run: async (interaction, client, args) => {
        ...
        for (let i = 0; i < usernames.length; i++) {
            ...
            let data = res.data;
            data.muteData.isMuted = false;
            data.muteData.reason = "";
            try {
            ...
            try {
                await MessagingService_1.default.sendMessage(universeID, "Unmute", { username: username, reason: reason });
            }
            catch (e) {
                didUnmuteError = true;
                logs.push({
                    username: username,
                    status: "Error",
                    message: `Although this user is now unmuted, I couldn't unmute them in the game because of the following error: ${e}`
                });
            }
            if (!didUnmuteError) {
                logs.push({
    ...
    slashData: new discord_js_1.default.SlashCommandBuilder()
        ...
        .addStringOption(o => o.setName("reason").setDescription("The reason(s) of the unmute(s)").setRequired(false)),
    commandData: {
        category: "Mute",
        isEphemeral: false,
