...
const command = {
    run: async (interaction, client, args) => {
        ...
        for (let i = 0; i < usernames.length; i++) {
            ...
            await client.logAction(`<@${interaction.user.id}> aceitou a solicitação de **${username}** pelo motivo de **${reason}** no grupo **${GroupHandler_1.default.getNameFromID(groupID)}**`);
            continue;
        }
    ...
    slashData: new discord_js_1.default.SlashCommandBuilder()
        ...
        .addStringOption(o => o.setName("group").setDescription("O grupo a aceitar a solicitação").setRequired(true).addChoices(...GroupHandler_1.default.parseGroups()))
        ...
        .addStringOption(o => o.setName("reason").setDescription("O (s) motivo (s) de por que você está aceitando as solicitações (s) de junção (s)").setRequired(false)),
    commandData: {
