...
const command = {
    run: async (interaction, client, args) => {
        ...
        for (let i = 0; i < usernames.length; i++) {
            ...
            await client.logAction(`<@${interaction.user.id}> promoveu **${username}** de **${oldRoleName}** para **${potentialRole.name}** pelo motivo de **${reason}** em **${GroupHandler_1.default.getNameFromID(groupID)}**`);
        }
    ...
    slashData: new discord_js_1.default.SlashCommandBuilder()
        ...
        .addStringOption(o => o.setName("group").setDescription("O grupo para fazer a promoção").setRequired(true).addChoices(...GroupHandler_1.default.parseGroups()))
        ...
        .addStringOption(o => o.setName("reason").setDescription("O (s) motivo (s) do (s) promoção (s)").setRequired(false)),
    commandData: {
        category: "Ranking",
        isEphemeral: false,
